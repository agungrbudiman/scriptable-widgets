...
if (session === true) {
  ...
  xl.api('balance').then((req) => {
    ...
    xl.api('quotadetails').then((req) => {
      ...
      widget.addTexts(`${com.byteToGiB(quota.global.used)} GB / ${com.byteToGiB(quota.global.remaining)} GB`, 14, Color.white(), 2)
      widget.addTexts(quota.local.info, 10, Color.lightGray())
      widget.addTexts(`${com.byteToGiB(quota.local.used)} GB / ${com.byteToGiB(quota.local.remaining)} GB`, 14, Color.white(), 2)
      widget.addTexts('Voice', 10, Color.lightGray())
      widget.addTexts(`${com.secToMin(quota.voice.used)} Min / ${com.secToMin(quota.voice.remaining)} Min`, 14, Color.white(), 5)
